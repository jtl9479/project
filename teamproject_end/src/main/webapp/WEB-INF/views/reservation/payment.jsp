									...
									<div class="single-sidebar-widget post-category-widget">
										<ul class="cat-list">
											...
											<li>
												<a class="d-flex justify-content-between">
													<p>시설명</p>
													<p><a href = "booking?finfo_no=${map.rVo.finfo_no }">${map.rVo.finfo_name }</a></p>
												</a>
											</li>
											<li>
												<a class="d-flex justify-content-between">
							...
							<colgroup width="80%">
								...
								<c:if test="${map.pVo.pay_status == '1' }">
									...
									<tr>
										...
										<td class = "info_text_td"><span class = "refund_span">${map.refundVo.refund_price }</span>원</td>
									</tr>
								</c:if>
							</table>
						</div>
							...
							<div class="cancel_outline" style="text-align: left;">
								<div id = "cancel_btn">
									<c:if test="${(session_id == map.mVo.m_id) && (map.pVo.pay_status != '1') }">
										...
										<button class="genric-btn primary"onclick = "reservatioPage()">예약페이지 이동</button>
								</div>
							</div>
						</div>
			</section>
