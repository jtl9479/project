...
public class admin_controller {
	...
	// 관리자 메인 페이지
	//공지사항 게시판,이벤트 게시판 결과값 가지고오기
	@GetMapping("admin_main")
	public String addmin_main(Model model) {
		//회원수,시설관리자 수
		Map<String,Object> map = adminService.selectAllMember();
		//공지사항 게시판 리스트 
		List<NoticeBoardVo> Noticelist=adminService.adminNoticeAllList();
		//이벤트 게시판 리스트
		List<EventBoardVo> Eventlist=adminService.adminEventAllList();
	...
	public String eventboard_list(Model model) {
		List<EventBoardVo> list=adminService.adminEventAllList();
		model.addAttribute("list",list);
